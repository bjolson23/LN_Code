...
#' @param df Dataframe to operate on (phone bivariate with cumulative columns)
...
#' @examples
#' bv_tbl <- phone_bv_cum_tbl(df = march_telo_bv)
#' @export
#' @import kableExtra


phone_bv_cum_tbl <- function(df, var_name = '', var_cnt = 1, ver_color = "#18B200", disc_color = "#FF6A6A",  width_col = "5cm", scroll_width = "900px"){
  ...
  cum_cols <- which(str_detect(colnames(df), "CumRate"))
  ...
  tbl_scroll <- kable(tbl_df, "html",
                      ...
                      align = c(rep('c', var_cnt), rep(c('r', 'r', 'c'), 8))) %>%
    ...
    column_spec(cum_cols, width_min = "3.75cm") %>%
